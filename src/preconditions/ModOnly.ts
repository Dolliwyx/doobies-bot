...
export class UserPrecondition extends Precondition {
	public override chatInputRun(interaction: Interaction) {
		if (
			...
			!(interaction.member?.permissions as PermissionsBitField)
				.toArray()
				.some((perms) => ['Administrator', 'ManageGuild', 'ModerateMembers'].includes(perms))
		) {
	...
	public override messageRun(message: Message) {
		if (
			...
			!message.member?.permissions.toArray().some((perms) => ['Administrator', 'ManageGuild', 'ModerateMembers'].includes(perms))
		) {
			return this.error({ message: 'You must be an administrator to use this command.', identifier: 'PermissionsMissing' });
		}
		return this.ok();
